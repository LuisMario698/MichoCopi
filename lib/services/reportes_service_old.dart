...
class ReportesService {
  ...
  static Future<Map<String, dynamic>> obtenerReporteInventario() async {
    try {
      ...
      final stockBajo = productos.where((p) => p.stock < 10).toList();
      
      // Productos sin stock
      final sinStock = productos.where((p) => p.stock == 0).toList();
      
      // Valor total del inventario
      final valorTotal = productos.fold<double>(0, (sum, p) => sum + (p.precio * p.stock));
      
      // Agrupar por categoría
      final porCategoria = <String, Map<String, dynamic>>{};
      for (final producto in productos) {
        final categoria = 'Categoría ${producto.idCategoria}'; // Simplificado por ahora
        ...
        porCategoria[categoria]!['valor'] += producto.precio * producto.stock;
  ...
  */
    ...
    try {
      ...
      return {
        ...
        'data': {
          ...
          'margenUtilidad': margenUtilidad,
          'ventasDetalle': ventasResult['success'] ? ventasResult['data'] : null,
          'comprasDetalle': comprasResult['success'] ? comprasResult['data'] : null,
        },
        'message': 'Resumen financiero generado exitosamente',
      };
    } catch (e) {
  ...
  static Future<Map<String, dynamic>> obtenerReporteProductos() async {
    try {
      ...
      final mayorValorInventario = [...productos]
        ..sort((a, b) => (b.precio * b.stock).compareTo(a.precio * a.stock));      // Agrupar por categoría
      ...
      for (final producto in productos) {
        ...
        porCategoria[catId]!['valorInventario'] += producto.precio * producto.stock;
  ...
  static String exportarProductosCSV(List<Producto> productos) {
    ...
    for (final producto in productos) {
      csv += '${producto.id},${producto.nombre},${producto.precio},${producto.stock},${producto.idCategoria}\n';
    }
    return csv;
  }
}
